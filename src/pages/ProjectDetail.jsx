  ...
  if (!project) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-4">
        ...
        <Link to="/projects" className="text-teal font-label font-semibold">← Back to Projects</Link>
      </div>
    )
  }
    ...
    <div>
      ...
      <section className="relative h-80 sm:h-[28rem] overflow-hidden flex items-end">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 pb-10 text-white w-full">
          ...
          <div className="flex items-center gap-2 text-teal-sage font-label text-xs uppercase tracking-widest2 mb-2">
        ...
        <div className="grid lg:grid-cols-3 gap-10">
          ...
          <div className="lg:col-span-2 space-y-10">
              ...
              <div className="grid grid-cols-3 gap-4">
                {[['Total Area', area], ['Plot Sizes', plotSizes], ['Starting Price', price]].map(([l, v]) => (
                  <div key={l} className="bg-white border border-sand-beige rounded-xl2 p-4 text-center">
                    <div className="font-display text-xl font-bold text-teal mb-1">{v}</div>
            ...
            <Reveal>
              ...
              <div className="flex flex-wrap gap-3">
                {approvals.map((a) => (
                  <span key={a} className="flex items-center gap-1.5 px-4 py-2 bg-teal/10 border border-teal/20 rounded-full text-sm font-label text-teal-dark">
                    <CheckCircle2 size={13} /> {a}
                    <span className="ml-1 text-xs bg-teal text-white px-2 py-0.5 rounded-full">Verified</span>
                  </span>
                ))}
              </div>
            </Reveal>
              ...
              <Reveal>
                ...
                <div className="overflow-x-auto rounded-xl2 border border-sand-beige">
                  <table className="w-full text-sm">
                    <thead style={{ background: '#514832' }} className="text-white">
                      <tr>
                      ...
                      {availability.map((row, i) => (
                        <tr key={i} className={i % 2 === 0 ? 'bg-white' : 'bg-offwhite'}>
                          ...
                          <td className="px-4 py-3">
                            <span className={`px-2 py-0.5 rounded-full text-xs font-label font-semibold ${row.status === 'Available' || row.status === 'Pre-Launch' ? 'bg-teal/10 text-teal-dark' : 'bg-charcoal/10 text-charcoal'}`}>
                              {row.status}
                            </span>
                          </td>
          ...
          <div>
            <div className="sticky top-24 bg-white border border-sand-beige rounded-xl3 p-6 shadow-sm space-y-4">
              ...
              <a href={CONTACT.tel} className="flex items-center justify-center gap-2 w-full py-3 rounded-full bg-teal text-white font-label font-semibold hover:bg-teal-dark transition-colors">
              ...
              <button className="flex items-center justify-center gap-2 w-full py-3 rounded-full border border-sand-beige text-charcoal font-label font-semibold hover:border-teal transition-colors">
